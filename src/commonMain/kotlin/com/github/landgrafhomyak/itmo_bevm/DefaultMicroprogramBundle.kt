...
private inline fun microprogram.goto(label: String) = CTRL(RDPS, LTOL, 0b00010000u, false, label)
private inline fun microprogram.ccr(bit: UByte, expected: Boolean, label: String) = CTRL(RDCR, if (bit >= 8u) HTOL else LTOL, 1u shl (if (bit >= 8u) (bit.toInt() - 8) else bit.toInt()), expected, label)

@Suppress("SpellCheckingInspection")
val defaultMicroprogramBundle = microprogram {
    null(1u)
    -"INFETCH"
    mov(RDIP, WRBR, WRAR)
